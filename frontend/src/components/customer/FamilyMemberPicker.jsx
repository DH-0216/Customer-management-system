 ...
 */
export default function FamilyMemberPicker({ value = [], onChange, excludeId }) {
  ...
  useEffect(() => {
    if (!value.length) { setSelected([]); return }
    ...
    Promise.all(missing.map((id) => customerApi.getById(id).then((r) => r.data)))
      .then((customers) => {
        setSelected((prev) => [
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      ...
      {open && (query.trim() || results.length > 0) && (
        ...
        >
          {loading && (
            <div style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontSize: '0.83rem' }}>
              Searching…
            </div>
          )}
          {!loading && results.length === 0 && query.trim() && (
            <div style={{ padding: '12px 16px', color: 'var(--text-muted)', fontSize: '0.83rem' }}>
              No customers found
            </div>
          )}
          {results.map((c) => (
